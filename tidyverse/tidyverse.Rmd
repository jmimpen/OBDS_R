...
> Answer: tibble, 150 x 5, dbl (decimal point numbers) and chr (characters)
...
```{r}
iris_raw %>% group_by(species) %>% summarise(Sepal.Length.mean = mean(sepal_length),
...
iris_raw %>% group_by(species) %>% summarise(across(where(is.numeric), mean)) %>% filter(sepal_length < 6)
...
#iris_raw$sepal_area <- iris_raw$sepal_length * iris_raw$sepal_width

iris_raw %>% mutate(sepal_area_new = sepal_length * sepal_width)
...
```{r}
# Copy the code chunk above and extend with more pipes
iris_raw %>% mutate(sepal_area_new = sepal_length * sepal_width) %>% select(species, sepal_area)
...
```{r}
iris_long <- iris_raw %>% 
  mutate(flower_ID = paste0(species, row.names(iris))) %>% 
...
```{r}
# Copy the code chunk above and refine to address the issue
iris_wide <- iris_long %>% pivot_wider(names_from = name, values_from = value)
...
```{r}
exprs_assay_long <- exprs_assay %>% pivot_longer(cols = where(is.numeric), names_to = "name", values_to = "expression")
...
```{r}
ggplot(exprs_assay_long, aes(x = expression)) +
  geom_histogram() +
