...
seurat_object <- CreateSeuratObject(read10x_data,
                                    project = "pbmc5k", #only really useful if you need to merge datasets
                                    min.cells = 3,
                                    min.features = 200)
...
```{r}
GetAssayData(seurat_object[["RNA"]], slot = "counts")[1:6,1:6]
...
class(seurat_object$orig.ident)
...
```{r}
FeatureScatter(seurat_object, feature1 = "percent_mt", feature2 = "nCount_RNA")
...
FeatureScatter(seurat_after_qc, feature1 = "percent_mt", feature2 = "nCount_RNA")
FeatureScatter(seurat_after_qc, feature1 = "nCount_RNA", feature2 = "percent_mt")
```
...
```{r}
lyz_raw <- FetchData(seurat_object, vars = c("LYZ", "nCount_RNA"))
lyz_normalised <- FetchData(seurat_after_qc, vars = c("LYZ", "nCount_RNA"))

ggplot_lyz_raw <- ggplot(lyz_raw, aes(x = LYZ)) +
    geom_histogram() +
    coord_cartesian(ylim = c(0, 500)) +
    cowplot::theme_cowplot()

ggplot_lyz_normalised <- ggplot(lyz_normalised, aes(x = LYZ)) +
    geom_histogram() +
    coord_cartesian(ylim = c(0, 500)) +
    scale_y_continuous(
...
remove(lyz_normalised, lyz_raw, ggplot_lyz_normalised, ggplot_lyz_raw)
```
...
seurat_after_qc_vf <- subset(x = seurat_after_qc,
                             features = VariableFeatures(object = seurat_after_qc))
...
seurat_after_qc <- ScaleData(seurat_after_qc, 
                             vars.to.regress = c("nCount_RNA", "percent_mt")) #regressing out library size and mitochondrial gene fraction to account for differences or variations caused by these two sources with are unwanted (or not biological variation)

#scale the number of variable genes you have set in the previous step, in this case 2000
```
...
```{r}
ElbowPlot(seurat_after_qc, ndims = 50) + coord_cartesian(ylim = c(0,15))
```
...
```{r}
seurat_after_qc <- RunUMAP(seurat_after_qc, reduction = "pca", dims = 1:20)
UMAPPlot(seurat_after_qc)

```

```{r}
saveRDS(seurat_after_qc, file = "Seuratobject_after_qc_20Feb2023.RDS")
```
...
DimPlot(seurat_after_qc, reduction = "umap", label = TRUE)
```
...
FeaturePlot(seurat_after_qc,
            features = c("GNLY", "NKG7", "FGFBP2", "PRF1"),
            label = TRUE)
```
...
gene_list_for_heatmap <- pull(markers_top10_clusters, gene)
    
DoHeatmap(seurat_after_qc, features = gene_list_for_heatmap)
```
