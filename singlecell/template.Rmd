...
VlnPlot(seurat_after_qc, features = c("nCount_RNA", "nCount_SCT"), group.by = "orig.ident", y.max = 70000)
VlnPlot(seurat_after_qc, features = c("nFeature_RNA", "nFeature_SCT"), group.by = "orig.ident", y.max = 8000)
```
...
ggplot_lyz_normalised <- ggplot(FetchData(seurat_after_qc,), aes(LYZ)) +
    geom_histogram(fill = "grey", color = "black", bins = 100) +
    coord_cartesian(ylim = c(0, 500)) +
    cowplot::theme_cowplot()
ggplot_lyz_scaled <- ggplot(FetchData(seurat_after_qc, ), aes(LYZ)) +
    geom_histogram(fill = "grey", color = "black", bins = 100) +
    coord_cartesian(ylim = c(0, 500)) +
    cowplot::theme_cowplot()
...
seurat_after_qc <- RunPCA(seurat_after_qc, reduction.name = "pca.sct")
...
```{r}
p3 <- DimPlot(seurat_after_qc, reduction = "umap.sct", group.by = "SCT_snn_res.0.5", label = TRUE) + ggtitle("SCT UMAP")
...
FeaturePlot(seurat_after_qc, features = c("LYZ","FCGR3A","CD4","CD8A","GNLY","MS4A1","CST3","PPBP"), reduction = "umap.sct")
...
seurat_markers_all <- FindAllMarkers(seurat_after_qc, assay = "RNA", logfc.threshold = 0.25, min.pct = 0.25)

seurat_markers_all_sct <- FindAllMarkers(seurat_after_qc, assay = "SCT", logfc.threshold = 0.25, min.pct = 0.25)
...
RidgePlot(hto12_object, features = c("KG1-A", "THP1-A", "K562-A", "HEK-A", "THP1-B"))
```
