...
```{r}
truseq_adapters <- readDNAStringSet("/t1-data/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/TruSeq3-PE-2.fa")
truseq_adapters

truseq_adapters2 <- readDNAStringSet("/t1-data/project/obds/jmimpen/base_r/projects/bioconductor/TruSeq3-PE-2-copy.fa")
truseq_adapters2
...
```{r}
gtf_data <- rtracklayer::import("/t1-data/project/obds/albrecht/3_r/9_bioconductor/data/Homo_sapiens.GRCh38.104.gtf.gz")
gtf_data
```
...
gtf_ACTB_transcriptnames <- subset(gtf_ACTB, type == "transcript")

gtf_ACTB_transcriptnames <- unique(gtf_ACTB_transcriptnames$transcript_id)
gtf_ACTB_transcriptnames
...
```{r}
?keys
human_symbols <- head(keys(org.Hs.eg.db, keytype = "SYMBOL"))
...
```{r}
AnnotationDbi::select(org.Hs.eg.db, keys = human_symbols, columns = c("ENTREZID", "ENSEMBL"), keytype = "SYMBOL")
...
demo_se <- SummarizedExperiment(
  assays = list(
    counts = demo_count_matrix), #counts = name of table
...
```{r}
assay_counts <- read.csv("/t1-data/project/obds/albrecht/3_r/9_bioconductor/data/summarized_experiment/counts.csv", row.names = 1)
...
```{r}
sample_metadata <- read.csv("/t1-data/project/obds/albrecht/3_r/9_bioconductor/data/summarized_experiment/sample_metadata.csv", row.names = 1)
head(sample_metadata)
```

```{r}
gene_metadata <- read.csv("/t1-data/project/obds/albrecht/3_r/9_bioconductor/data/summarized_experiment/gene_metadata.csv", row.names = 1)
head(gene_metadata)
```
