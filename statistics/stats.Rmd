...
ggplot(df, aes(x = x)) + geom_histogram(binwidth = 1, fill = "grey", colour = "black")
```
...
ggplot(df, aes(x = x)) + 
  ...
  geom_vline(xintercept = (median(x)), color = "red", linetype = "dashed") +
  geom_vline(xintercept = (mean(x) - sd(x)), colour = "blue") +
  geom_vline(xintercept = (mean(x) + sd(x)), colour = "blue") +
  geom_vline(xintercept = (median(x) - mad(x)), colour = "blue", linetype = "dashed") +
  geom_vline(xintercept = (median(x) + mad(x)), colour = "blue", linetype = "dashed")
...
ggplot(df_y, aes(x = y)) + 
  ...
  geom_vline(xintercept = (median(y)), color = "red", linetype = "dashed") +
  geom_vline(xintercept = (mean(y) - sd(y)), colour = "blue") +
  geom_vline(xintercept = (mean(y) + sd(y)), colour = "blue") +
  geom_vline(xintercept = (median(y) - mad(y)), colour = "blue", linetype = "dashed") +
  geom_vline(xintercept = (median(y) + mad(y)), colour = "blue", linetype = "dashed")
...
plot1 <- ggplot(data.frame(x = c(-5, 5)), aes(x = x)) + stat_function(fun = pnorm)
plot1

vec1 <- seq(from = -5, to = 5, by = 0.1)
vec2 <- pnorm(q = vec1, mean = 0, sd = 1)
...
plot1 <- ggplot(data.frame(x = c(0, 1)), aes(x = x)) + stat_function(fun = qnorm)
plot1
...
wilcox.test(vector1, vector2, paired = TRUE)
...
T-test
Import the data in the fi le
gene_exprs.csv
.
Run a t-test to compare the gene expressionvalues between the two groups.
What is the p-value?
```{r}
gene_exprs <- read_csv("/project/obds/shared/resources/3_r_stats_genomics/statistics/data/gene_exprs.csv")
summary(gene_exprs)

unique(gene_exprs$group)

#Use if only two groups in object
t.test(gene_exprs ~ group, data = gene_exprs)

#Use if more than two groups or if groups need to be specified
t.test(
  x = gene_data$gene_exprs[gene_data$group == "groupA"], #x is test 
  y = gene_data$gene_exprs[gene_data$group == "groupB"]) #y is control group, if not specified then it is zero
...
```{r}
excel_sheets("/t1-data/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx")
...
cell_info <- read_excel("/t1-data/project/obds/shared/resources/3_r_stats_genomics/statistics/data/GSE111543.xlsx", sheet = "cell_info")

unique(cell_info$Infection)
unique(cell_info$Sample)

cells_infected <- cell_info %>% filter(Infection == "STM-LT2") %>% pull(Sample)
cells_infected

cells_not_infected <- cell_info %>% filter(Infection == "Mock") %>% pull(Sample)
cells_not_infected
...
gene1_test <- t.test(expression_infected, expression_not_infected)
gene1_test
...
gene_test <- t.test(expression_infected, expression_not_infected)

return(gene_test$p.value)
}
...
t_test_pvalues <- vapply(X = row.names(logcounts_matrix), 
                         FUN = t_test_row,
                         FUN.VALUE = numeric(1))
